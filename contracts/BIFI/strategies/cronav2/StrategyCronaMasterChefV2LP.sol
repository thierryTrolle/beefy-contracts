...
import "../../interfaces/cronav2/ICronaMasterChefV2LP.sol";
import "../../interfaces/cronav2/ICronaMasterChefV2StakingReward.sol";
import "../Common/StratManager.sol";
import "../Common/FeeManager.sol";

import "hardhat/console.sol";

contract StrategyCronaMasterChefV2LP is StratManager, FeeManager {
   using SafeERC20 for IERC20;
    ...
    constructor(
        ...
        console.log("outputToLp0Route[last]",_outputToLp0Route[_outputToLp0Route.length - 1]);
    ...
    function deposit() public whenNotPaused {
        ...
        if (wantBal > 0) {
            ICronaMasterChefV2LP(chef).deposit(poolId, wantBal);
    ...
    function withdraw(uint256 _amount) external {
        ...
        if (wantBal < _amount) {
            ICronaMasterChefV2LP(chef).withdraw(poolId, _amount.sub(wantBal));
    ...
    function _harvest(address callFeeRecipient) internal whenNotPaused {
        // ICronaMasterChefV2LP(chef).harvest(poolId, address(this));//fixme
        ICronaMasterChefV2LP(chef).harvest();
    ...
    function balanceOfPool() public view returns (uint256) {
        (uint256 _amount, ) = ICronaMasterChefV2LP(chef).userInfo(poolId, address(this));
        return _amount;
    }
    ...
    function retireStrat() external {
        ...
        ICronaMasterChefV2LP(chef).emergencyWithdraw(poolId);
    ...
    function rewardsAvailable() public view returns (uint256) { 
        return ICronaMasterChefV2LP(chef).pendingCrona(poolId, address(this));
    }
    ...
    //     uint256 cronaReward = ICronaMasterChefV2LP(chef).pendingCrona(poolId, address(this));
    //     uint256[] memory secondaryRewards;
    //     // checks if there is a rewarder associated with the pool, if not will return an empty array.
    //     address rewarder = ICronaMasterChefV2LP(chef).rewarder(poolId);
    //     if (rewarder != nullAddress) {
    //     (, uint256[] memory amounts) = ICronaMasterChefV2StakingReward(rewarder).pendingTokens(poolId, address(this), cronaReward);
    //         secondaryRewards = amounts;
    //     }

    //     return (cronaReward, secondaryRewards);
    // }

    // function callReward() public view returns (uint256) {
    //     (uint256 cronaReward, uint256[] memory secondaryRewards) = rewardsAvailable();
    //     uint256 nativeBal;
    //     try IUniswapRouterETH(unirouter).getAmountsOut(cronaReward, outputToNativeRoute)
